...
- 🚀 **自动启动：** 伴侣启动时可自动拉起目标代理，并自动应用当前选中主题
...
2. **启动代理：** 点击「启动代理」按钮（或开启「自动启动」开关，下次启动伴侣时自动拉起）
3. **选择主题：** 在主题网格中点击任意主题卡片即可预览并应用
4. **切换开关：** 「主题开关」控制是否注入主题样式，关闭后恢复代理原始界面

### 为非调试模式代理应用主题

如果代理已经在运行但未开启 CDP 端口，Agent Theme 无法注入主题。解决方法：

1. 在 Agent Theme 中先关闭代理进程
2. 通过 Agent Theme 的「启动代理」按钮重新启动（会自动附加 `--remote-debugging-port` 参数）
3. 启动后自动应用当前选中主题
...
确保代理是由 Agent Theme 启动的（而非手动启动），这样代理才会携带 `--remote-debugging-port` 参数暴露 CDP 端口。检查界面上的 CDP 端口状态是否为「已绑定」。
...
CDP 注入的主题在代理重启后会丢失。Agent Theme 会在检测到代理启动后自动重新注入，保持「自动启动」和「主题开关」开启即可自动恢复。
